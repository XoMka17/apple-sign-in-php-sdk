...
/**
 * Base Class for all \phpseclibnzr3\Crypt\* cipher classes
 *
 * PHP version 5
 *
 ...
 *
 *  - The new \phpseclibnzr3\Crypt\* cipher class should extend \phpseclibnzr3\Crypt\Common\SymmetricKey
 *
 ...
 *
 *  - Look at the source code of the current ciphers how they extend \phpseclibnzr3\Crypt\Common\SymmetricKey
...
namespace phpseclibnzr3\Crypt\Common;

use phpseclibnzr3\Common\Functions\Strings;
use phpseclibnzr3\Crypt\Hash;
use phpseclibnzr3\Exception\BadDecryptionException;
use phpseclibnzr3\Exception\BadModeException;
use phpseclibnzr3\Exception\InconsistentSetupException;
...
use phpseclibnzr3\Math\BinaryField;
use phpseclibnzr3\Math\PrimeField;
...
abstract class SymmetricKey
{
     ...
     * @access public
     * @see \phpseclibnzr3\Crypt\Common\SymmetricKey::encrypt()
     * @see \phpseclibnzr3\Crypt\Common\SymmetricKey::decrypt()
     */
    const MODE_CTR = -1;
    /**
     ...
     *
     * @access private
     * @see \phpseclibnzr3\Crypt\Common\SymmetricKey::__construct()
     */
    const MODE_MAP = [
        'ctr'    => self::MODE_CTR,
        'ecb'    => self::MODE_ECB,
     ...
     *
     * @access private
     * @see \phpseclibnzr3\Crypt\Common\SymmetricKey::getEngine()
     */
    const ENGINE_MAP = [
     ...
     *
     * @see \phpseclibnzr3\Crypt\Twofish::setKey()
     * @see \phpseclibnzr3\Crypt\Twofish::setIV()
     * @var bool
     * @access private
     */
     ...
     * @access private
     * @param int $n
     * @param \phpseclibnzr3\Crypt\Hash $hashObj
     * @param string $i
     * @param string $d
     * @param int $count
     * @return string $a
     ...
     *
     * Put another way, when the continuous buffer is enabled, the state of the \phpseclibnzr3\Crypt\*() object changes after each
     ...
     * Encrypts a block
     *
     * Note: Must be extended by the child \phpseclibnzr3\Crypt\* class
     *
     * @access private
     * @param string $in
     * @return string
     ...
     *
     * Note: Must extend by the child \phpseclibnzr3\Crypt\* class
     *
     * @see self::setup()
     * @access private
     */
     ...
     *       - In case of using inline crypting, _setupInlineCrypt() must extend by the child \phpseclibnzr3\Crypt\* class.
     ...
     *    </code>
     *
     *    See also the \phpseclibnzr3\Crypt\*::_setupInlineCrypt()'s for
                    ...
                    if (strlen($_buffer["ciphertext"])) {
                        for ($_i = 0; $_i < $_plaintext_len; $_i+= ' . $block_size . ') {
                            ...
                            if (strlen($_block) > strlen($_buffer["ciphertext"])) {
                                $in = $_xor;
                                ' . $encrypt_block . '
                                \phpseclibnzr3\Common\Functions\Strings::increment_str($_xor);
                                $_buffer["ciphertext"].= $in;
                            }
                            $_key = \phpseclibnzr3\Common\Functions\Strings::shift($_buffer["ciphertext"], ' . $block_size . ');
                ...
                $encrypt = $init_encrypt . '
                    ...
                    if (strlen($_buffer["xor"])) {
                        for ($_i = 0; $_i < $_plaintext_len; $_i+= ' . $block_size . ') {
                            ...
                            $_key = \phpseclibnzr3\Common\Functions\Strings::shift($_buffer["xor"], ' . $block_size . ');
                            $_ciphertext.= $_block ^ $_key;
                        }
                    } else {
                        for ($_i = 0; $_i < $_plaintext_len; $_i+= ' . $block_size . ') {
